...
*
* (c) 2019 unix-world.org - fixes and optimizations (removed 3d board since it had many bugs ...)
*
*/
...
var chess = (function() {
	...
	oGameInfo = {}, oNewInfo = {}, etc = { // do not change theese values!!
		...
		makeSelection: function(nSquareId) {
			if (!bReady) { return; }
	...
	*/
	...
	function writeFlatPieces() {
		...
		for (var iCell = 0; iCell < 64; iCell++) {
			...
			if (nSquareId === lastStart || nSquareId === lastEnd) { oSquareCell.style.backgroundColor = (nSquareId * 11 - nSquareId % 10) / 10 & 1 ? "#D6FBC9" : "#CAFBB2"; } else { oSquareCell.style.backgroundColor = ""; }
		}
	...
	function squareFocus(nPieceId, bMakeActive) {
		...
		oSelCell.style.backgroundColor = bMakeActive ? "#FFCC00" : sLstSqColr;
	}
...
// DOM private APIs
	...
	function getSqFnc() {
		...
		etc.makeSelection(etc.bBlackSide ? 119 - getId : getId);
	}
	...
	function updateViewSize(bCrushFlatWidth) {
...
// Public APIs
	return {
		...
		setView: function(nView) {
			if (!bReady) { return(false); }
			var bUpdateSize = false,
			bShow2D = Boolean(nView & 1),
			bChanged2D = Boolean(nView & 1 ^ etc.bFlatView);
			if (bShow2D && nDeskWidth < nMinHeight << 1) {
		...
		setDimensions: function(nNewWidth, nNewHeight) {
			...
			nDeskHeight = nNewHeight < nMinHeight ? nMinHeight : nNewHeight - 1 | 1;
